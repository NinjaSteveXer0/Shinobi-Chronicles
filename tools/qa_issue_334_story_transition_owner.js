...
assert(sharedPlay.includes('cancelStoryHardSceneTransition33900("superseded")'),"stale shared transition is not invalidated");
...
assert(!renderer.includes(".kv2-transition-memory")&&!renderer.includes(".kv2-wipe"),"36030 retained alternate root transition layer");
...
assert(!renderer.includes("cloneNode(")&&!transition.includes("cloneNode("),"transition migration introduced clone/ghost ownership");
...
assert((transitionRow.retiredOwners||[]).some(x=>String(x).includes("ensureGlobalCurtain")),"36030 curtain retirement not registered");
assert((transitionRow.retiredOwners||[]).some(x=>String(x).includes("36040")&&String(x).includes("playback")),"36040 playback retirement not registered");
