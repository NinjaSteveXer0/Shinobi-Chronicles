...
const runtimeSource=fs.readFileSync(path.resolve(process.cwd(),"runtime/alpha-kakashi-move-closer-closure-35930.js"),"utf8");
assert(runtimeSource.includes("PICKPOCKET_IMPROVED_SUCCESS_CLEAN_EXTRACTION"));
assert(runtimeSource.includes("You sound disappointed."));
assert(runtimeSource.includes("You're taking me back without the package."));
assert(runtimeSource.includes("Pakkun remains a witness where factually present."));
assert(runtimeSource.includes("kak_move_closer_improved_pickpocket_success_35930"),"improved-pickpocket success must have a player-facing beat");
...
assert.strictEqual(choice(SUCCESS,"strike_before_handoff").nextBeatId,MOD.beats.successStrikePrelude);
...
active.beatId=MOD.beats.groupKill;
...
active.beatId=MOD.beats.psRestrain;
...
active.beatId=MOD.beats.groupRelease;
...
assert.notStrictEqual(snap.participantStates[PAKKUN]&&snap.participantStates[PAKKUN].stateClass,"DEPARTED","two-target disposition must preserve Pakkun until an explicit authored departure");
...
console.log("- two-target group dispositions preserve participant-level state while Pakkun remains until an explicit authored departure");
