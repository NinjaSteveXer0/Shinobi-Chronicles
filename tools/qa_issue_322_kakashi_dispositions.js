...
// Shared producer: committed-source gate + same-causal-root upsert instead of fake breadth.
{
  ...
  const firstPacket={...base,sourceOccurrenceId:"src1",tags:["covert_operations.extraction_specialist:extraction_planning"],significance:1,capabilitySnapshot:{buki:7}};
  const firstProjection=ctx.projectSpecialJoninContextualEvidence34700(firstPacket);
  assert(firstProjection.success);
  const retryProjection=ctx.projectSpecialJoninContextualEvidence34700(firstPacket);
  assert(retryProjection.success&&retryProjection.idempotent===true,"identical producer retry was not idempotent");
  const retryRows=ctx.getSpecialJoninContextualEvidence34700("academy_kakashi",definition.qualificationId);
  assert.strictEqual(retryRows[0].context.capabilitySnapshots.length,1,"identical retry duplicated capability snapshot");
