...
const PATCH_ID="kakashi_scene_board_model_v8_33910_2026_09_19";
...
function transferProjection(performance,context){
  ...
  if(expanded){
    ...
    let actors=[];
    if(idx<=11){
      actors=[kakashi("OBSERVING · UNSEEN",focus==="academy_kakashi"),amt(idx>=6?"LEAVING EXCHANGE":"AT EXCHANGE",focus==="anbu_marked_target"),smuggler(psState,focus==="package_smuggler")];
    }else if(idx<=14){
      const amtState=idx===14?"BREAKING AWAY":"LEAVING EXCHANGE";
      const miState=idx>=13?"PRESSING PACKAGE SMUGGLER":"BURSTING FROM SHADOW";
      actors=[amt(amtState,focus==="anbu_marked_target"),interceptor(miState,focus==="masked_interceptor",idx===12),smuggler(psState,focus==="package_smuggler")];
    }else{
      const miState=idx>=16?"CUTTING OFF ESCAPE":"PRESSING PACKAGE SMUGGLER";
      actors=[kakashi("OBSERVING · UNSEEN",focus==="academy_kakashi"),interceptor(miState,focus==="masked_interceptor"),smuggler(psState,focus==="package_smuggler")];
    }
...
function speakerActorId(name){const key=String(name||"").trim().toUpperCase();if(key==="KAKASHI"||key==="KAKASHI HATAKE")return"academy_kakashi";if(key==="ANBU OPERATIVE")return"konoha_anbu_contact";if(key==="ANBU MARKED TARGET")return"anbu_marked_target";if(key==="PACKAGE SMUGGLER")return"package_smuggler";if(key==="MASKED INTERCEPTOR")return"masked_interceptor";if(key==="MINATO"||key==="MINATO NAMIKAZE")return"kage_minato";if(key==="PAKKUN")return"pakkun";return null;}
...
globalThis.SC_KAKASHI_SCENE_BOARD_POLISH_33910=Object.freeze({patchId:PATCH_ID,model:"scene_board_v8",scene01Authority:SCENE_01_AUTHORITY,scene02Authority:SCENE_02_AUTHORITY,browserGoldenClaimed:false});
