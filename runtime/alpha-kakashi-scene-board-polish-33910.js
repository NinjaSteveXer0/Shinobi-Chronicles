...
function speakerActorId(name){const key=String(name||"").trim().toUpperCase();if(key==="KAKASHI"||key==="KAKASHI HATAKE")return"academy_kakashi";if(key==="ANBU OPERATIVE")return"konoha_anbu_contact";if(key==="ANBU MARKED TARGET")return"anbu_marked_target";if(key==="PACKAGE SMUGGLER")return"package_smuggler";if(key==="MASKED INTERCEPTOR")return"masked_interceptor";if(key==="PAKKUN")return"pakkun";return null;}
...
function previousDialogue(performance){if(!performance||!Array.isArray(performance.sequence))return null;const current=performance.cue||{},currentSpeaker=String(current.speakerName||"");for(let i=performance.index-1;i>=0;i--){const cue=performance.sequence[i];if(!cue)continue;if(cue.kind!=="dialogue")break;if(cue.speakerName&&String(cue.speakerName)!==currentSpeaker)return cue;}return null;}
...
function makeDialoguePanel(cue,isCurrent,projection){const panel=document.createElement("section"),side=actorSideForSpeaker(cue&&cue.speakerName,projection);panel.className=`sc-dialogue-panel-33910 is-${side} ${isCurrent?"is-current":"is-previous"}`;const speaker=document.createElement("div");speaker.className="sc-dialogue-speaker-33910";speaker.textContent=escapeText(cue&&cue.speakerName||"DIALOGUE");const copy=document.createElement("div");copy.className="sc-dialogue-copy-33910";copy.textContent=escapeText(cue&&cue.text||"");const status=document.createElement("div");status.className="sc-dialogue-status-33910";status.textContent=isCurrent?"SPEAKING":"PREVIOUS";panel.append(speaker,copy,status);if(isCurrent)panel.appendChild(makeAdvanceButton());return panel;}
