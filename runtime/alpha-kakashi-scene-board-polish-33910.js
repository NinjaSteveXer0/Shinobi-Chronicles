...
const PATCH_ID="kakashi_scene_board_model_v14_33910_2026_09_21";
...
function syncPresentation(){if(typeof document==="undefined")return false;const layer=document.getElementById("story-scene-presentation-layer");if(!layer)return false;const runtime=currentRuntime();if(!runtime||runtime.sceneId!==scene){delete layer.dataset.scBoardUiMode;return false;}const stage=layer.querySelector(".sc-chronicle-stage")||layer.querySelector(".sc-story-stage")||layer;bindStageAdvance33910(stage,layer);const beat=currentBeat(runtime),performance=typeof getStoryScenePerformance33900==="function"?getStoryScenePerformance33900():null,projection=currentProjection();let mode="narration";if(performance&&performance.cue)mode=isDialogueCue33910(performance.cue)?"dialogue":"performance_narration";else if(beat&&beat.mode==="choice")mode="decision";layer.dataset.scBoardUiMode=mode;if(mode==="dialogue"||mode==="performance_narration")renderPerformanceSurface(layer,stage,performance,projection);else clearPerformanceSurface(stage);if(mode==="decision")decorateChoiceDeck(layer,beat);return true;}
