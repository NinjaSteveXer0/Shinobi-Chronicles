...
const PATCH_ID="kakashi_scene_board_model_v22_33910_2026_09_22";
...
#story-scene-presentation-layer .sc-scene-board-33900__objective{min-width:310px!important;max-width:min(52%,460px)!important;min-height:62px!important;padding:11px 15px!important;display:flex!important;flex-direction:column!important;justify-content:center!important;box-sizing:border-box!important;align-items:center!important;text-align:center!important;font-size:18px!important;line-height:1.28!important;box-shadow:0 11px 28px rgba(0,0,0,.36)!important;}
#story-scene-presentation-layer .sc-scene-board-33900__objective b{width:100%!important;text-align:center!important;font-size:11.5px!important;margin-bottom:6px!important;}
...
#story-scene-presentation-layer[data-sc-board-ui-mode="battle_transition"] .sc-chronicle-layout{width:min(76%,1040px)!important;margin:0 auto 3.3%!important;gap:10px!important;z-index:10!important;}
#story-scene-presentation-layer[data-sc-board-ui-mode="battle_transition"] .sc-story-panel{max-height:21vh!important;padding:13px 18px!important;position:relative!important;overflow:hidden!important;}
#story-scene-presentation-layer[data-sc-board-ui-mode="battle_transition"] .sc-chronicle-actions{margin-top:10px!important;}

.sc-kakashi-entry-curtain-33910{position:fixed;inset:0;background:#000;z-index:2147483000;pointer-events:auto;opacity:1;transition:opacity .30s ease;}.sc-kakashi-entry-curtain-33910.is-revealing{opacity:0;}
...
#story-scene-presentation-layer[data-sc-board-ui-mode="dialogue"] .sc-dialogue-panel-33910.is-current.is-left,#story-scene-presentation-layer[data-sc-board-ui-mode="dialogue"] .sc-dialogue-panel-33910.is-current.is-right{left:50%!important;right:auto!important;top:4%!important;bottom:auto!important;transform:translateX(-50%)!important;width:min(38%,540px)!important;max-width:540px!important;}
...
function directPickpocketReturnProjection33910(performance){
 ...
 if(defeated){
  if(performance&&performance.cue){
   const idx=Number(performance.index||0),actors=[kakashi("DEFEATED",true)];
   if(idx<=3)actors.push(smuggler(idx===3?"LEAVING":"PRESENT",false,false,idx===3?"exit":null));
   if(idx<=4)actors.push(amt(idx===4?"ESCAPING WITH PACKAGE":"PACKAGE CARRIER",false,false,idx===4?"exit":null));
   if(idx<=6)actors.push(interceptor(idx===6?"LEAVING":idx===5?"LINGERING":"PRESENT",false,false,idx===6?"exit":null));
   return{mode:"consequence",location:"KONOHA ALLEY",objective:"Return to ANBU.",actors,objects:[]};
  }
  return{mode:"consequence",location:"KONOHA ALLEY",objective:"Return to ANBU.",actors:[kakashi("DEFEATED",true)],objects:[]};
 }
...
function directPickpocketDispositionProjection33910(beatId,performance){
 const cueText=String(performance&&performance.cue&&performance.cue.text||""),index=Number(performance&&performance.index||0),sequence=performance&&Array.isArray(performance.sequence)?performance.sequence:[];
 if(beatId===DIRECT_PICKPOCKET_POLICE_34710)return{mode:"consequence",location:"UCHIHA POLICE FORCE · NIGHT",objective:"Transfer the three defeated shinobi to the Uchiha Police Force.",actors:[kakashi("REPORTING",true),amt("RESTRAINED"),smuggler("RESTRAINED"),interceptor("RESTRAINED")],objects:[{label:"PACKAGE",state:"SECURED BY KAKASHI"}]};
 if(beatId===DIRECT_PICKPOCKET_ANBU_34710)return{mode:"conversation",location:"KONOHA ROOFTOP · NIGHT",objective:"Transfer the three defeated shinobi to ANBU.",actors:[anbu("RECEIVING CUSTODY",true),kakashi("REPORTING"),amt("RESTRAINED"),smuggler("RESTRAINED"),interceptor("RESTRAINED")],objects:[{label:"PACKAGE",state:"RETURNING TO ANBU"}]};
 if(beatId===DIRECT_PICKPOCKET_KILL_34710){
  const fallIndex=sequence.findIndex(row=>String(row&&row.text||"").startsWith("When it is finished")),fall=fallIndex>=0&&index===fallIndex,finished=fallIndex>=0&&index>fallIndex;
  return{mode:"consequence",location:"KONOHA ALLEY",objective:"Return to ANBU.",actors:finished?[kakashi("PACKAGE SECURED",true)]:[kakashi("FINAL DECISION",true),amt("DEFEATED",false,false,fall?"fall":null),smuggler("DEFEATED",false,false,fall?"fall":null),interceptor("DEFEATED",false,false,fall?"fall":null)],objects:[{label:"PACKAGE",state:"SECURED BY KAKASHI"}]};
 }
 if(beatId===DIRECT_PICKPOCKET_RELEASE_34710){
  const exitIndex=sequence.findIndex(row=>String(row&&row.text||"").startsWith("One by one, they leave the street.")),exit=exitIndex>=0&&index===exitIndex,gone=exitIndex>=0&&index>exitIndex;
  return{mode:"consequence",location:"KONOHA ALLEY",objective:"Return to ANBU.",actors:gone?[kakashi("PACKAGE SECURED",true)]:[kakashi("PACKAGE SECURED",true),amt(exit?"LEAVING":"RELEASED",false,false,exit?"exit":null),smuggler(exit?"LEAVING":"RELEASED",false,false,exit?"exit":null),interceptor(exit?"LEAVING":"RELEASED",false,false,exit?"exit":null)],objects:[{label:"PACKAGE",state:"SECURED BY KAKASHI"}]};
 }
 return null;
}
function battleResume33910(context=null){
 const supplied=context&&context.battleResume&&context.battleResume.authored;
 if(supplied&&typeof supplied==="object")return supplied;
 try{const rt=currentRuntime();return rt&&rt.battleResume&&rt.battleResume.authored&&typeof rt.battleResume.authored==="object"?rt.battleResume.authored:null;}catch(_error){return null;}
}
function battleOutcome33910(context=null){
 const row=battleResume33910(context);
 return{row,win:!!row&&String(row.resultState||"")==="player_side_victory",loss:!!row&&String(row.resultState||"")==="opposition_side_victory",turns:row&&Number.isFinite(Number(row.playerActionOpportunityCount))?Number(row.playerActionOpportunityCount):null};
}
function sequentialProjection33910(beatId,context,performance){
 const result=battleOutcome33910(context),focus=String(performance&&performance.cue&&performance.cue.focusActorRef||"");
 const k=(state="ACTIVE")=>kakashi(state,focus==="academy_kakashi"||!focus);
 const p=(state="PRESENT",entering=false)=>pakkun(state,focus==="pakkun",entering);
 if(beatId==="kak_observe_secure_package_battle")return{mode:"battle_transition",location:"SAKURA TREE · MAIN STREET",objective:"Secure the package.",actors:[k("ENGAGING"),smuggler("PACKAGE CARRIER"),interceptor("CONTESTING PACKAGE")],objects:[{label:"PACKAGE",state:"WITH PACKAGE SMUGGLER"}]};
 if(beatId==="kak_observe_secure_package_return"){
  if(result.win)return{mode:"consequence",location:"SAKURA TREE · MAIN STREET",objective:"Choose whether to stay on the original target or return and report.",actors:[kakashi("PACKAGE SECURED",true),smuggler("BATTLE-DEFEATED"),interceptor("BATTLE-DEFEATED")],objects:[{label:"PACKAGE",state:"SECURED BY KAKASHI"}]};
  if(result.loss)return{mode:"consequence",location:"SAKURA TREE · MAIN STREET",objective:"Return and report.",actors:[kakashi("WITHDRAWING",true),smuggler("PACKAGE CARRIER"),interceptor("PRESENT")],objects:[{label:"PACKAGE",state:"WITH PACKAGE SMUGGLER"}]};
  return{mode:"consequence",location:"SAKURA TREE · MAIN STREET",objective:"Resolve the Battle return.",actors:[kakashi("ACTIVE",true),smuggler("PACKAGE CARRIER"),interceptor("PRESENT")],objects:[{label:"PACKAGE",state:"WITH PACKAGE SMUGGLER"}]};
 }
 if(beatId==="kak_observe_secure_package_pakkun_arrival")return{mode:"conversation",location:"KONOHA ALLEY · NIGHT",objective:"Stay on the original target.",actors:[k("PACKAGE SECURED · PURSUING"),amt("PURSUIT TARGET"),p("BLOCKING THE ROUTE",true)],objects:[{label:"PACKAGE",state:"SECURED BY KAKASHI"}]};
 if(beatId==="kak_observe_secure_package_amt_battle")return{mode:"battle_transition",location:"KONOHA ALLEY · NIGHT",objective:"Stop the original target.",actors:[kakashi("PACKAGE SECURED · ACTIVE",true),amt("CONTESTING"),pakkun("TEMPORARY ALLY")],objects:[{label:"PACKAGE",state:"SECURED BY KAKASHI"}]};
 if(beatId==="kak_observe_secure_package_amt_return"){
  if(result.win)return{mode:"consequence",location:"KONOHA ALLEY · NIGHT",objective:"Return and report.",actors:[kakashi("PACKAGE SECURED",true),amt("BATTLE-DEFEATED"),pakkun("PRESENT")],objects:[{label:"PACKAGE",state:"SECURED BY KAKASHI"}]};
  if(result.loss)return{mode:"consequence",location:"KONOHA ALLEY · NIGHT",objective:"Return and report.",actors:[kakashi("WITHDRAWING · PACKAGE SECURED",true),amt("PRESENT"),pakkun("PRESENT")],objects:[{label:"PACKAGE",state:"SECURED BY KAKASHI"}]};
  return{mode:"consequence",location:"KONOHA ALLEY · NIGHT",objective:"Resolve the Battle return.",actors:[kakashi("PACKAGE SECURED",true),amt("PRESENT"),pakkun("PRESENT")],objects:[{label:"PACKAGE",state:"SECURED BY KAKASHI"}]};
 }
 if(beatId==="kak_seq_mi_battle")return{mode:"battle_transition",location:"SAKURA TREE · MAIN STREET",objective:"Stop Masked Interceptor without losing the package trail.",actors:[kakashi("ACTIVE",true),interceptor("BLOCKING KAKASHI"),smuggler("ESCAPING WITH PACKAGE")],objects:[{label:"PACKAGE",state:"WITH PACKAGE SMUGGLER"}]};
 if(beatId==="kak_seq_mi_return"){
  const fast=result.win&&result.turns!==null&&result.turns<=4;
  if(result.loss)return{mode:"consequence",location:"SAKURA TREE · MAIN STREET",objective:"Return and report.",actors:[kakashi("DEFEATED · WITHDRAWING",true),interceptor("PRESENT"),smuggler("ESCAPING WITH PACKAGE")],objects:[{label:"PACKAGE",state:"WITH PACKAGE SMUGGLER"}]};
  if(result.win)return{mode:"consequence",location:"SAKURA TREE · MAIN STREET",objective:fast?"Choose whether to chase Package Smuggler or return and report.":"Return and report.",actors:[kakashi("ACTIVE",true),interceptor("BATTLE-DEFEATED"),smuggler(fast?"STILL REACHABLE · HAS PACKAGE":"ESCAPED WITH PACKAGE")],objects:[{label:"PACKAGE",state:fast?"WITH PACKAGE SMUGGLER · REACHABLE":"ESCAPED WITH PACKAGE SMUGGLER"}]};
  return{mode:"consequence",location:"SAKURA TREE · MAIN STREET",objective:"Resolve the Battle return.",actors:[kakashi("ACTIVE",true),interceptor("PRESENT"),smuggler("HAS PACKAGE")],objects:[]};
 }
 if(beatId==="kak_seq_ps_battle")return{mode:"battle_transition",location:"KONOHA ALLEY · NIGHT",objective:"Recover the package.",actors:[kakashi("ACTIVE",true),smuggler("PACKAGE CARRIER")],objects:[{label:"PACKAGE",state:"WITH PACKAGE SMUGGLER"}]};
 if(beatId==="kak_seq_ps_return"){
  const fast=result.win&&result.turns!==null&&result.turns<=3;
  if(result.loss)return{mode:"consequence",location:"KONOHA ALLEY · NIGHT",objective:"Return and report.",actors:[kakashi("DEFEATED · WITHDRAWING",true),smuggler("PRESENT")],objects:[]};
  if(result.win)return{mode:"consequence",location:"KONOHA ALLEY · NIGHT",objective:fast?"Choose whether to stay on the original target or return and report.":"Return and report.",actors:[kakashi("ACTIVE",true),smuggler("BATTLE-DEFEATED"),amt(fast?"STILL REACHABLE":"BEYOND REACH")],objects:[]};
  return{mode:"consequence",location:"KONOHA ALLEY · NIGHT",objective:"Resolve the Battle return.",actors:[kakashi("ACTIVE",true),smuggler("PRESENT")],objects:[]};
 }
 if(beatId==="kak_seq_pakkun_arrival")return{mode:"conversation",location:"KONOHA ALLEY · NIGHT",objective:"Stay on the original target.",actors:[k("PURSUING"),amt("PURSUIT TARGET"),p("BLOCKING THE ROUTE",true)],objects:[]};
 if(beatId==="kak_seq_amt_battle")return{mode:"battle_transition",location:"KONOHA ALLEY · NIGHT",objective:"Stop the original target.",actors:[kakashi("ACTIVE",true),amt("CONTESTING"),pakkun("TEMPORARY ALLY")],objects:[]};
 if(beatId==="kak_seq_amt_return"){
  if(result.win)return{mode:"consequence",location:"KONOHA ALLEY · NIGHT",objective:"Return and report.",actors:[kakashi("ACTIVE",true),amt("BATTLE-DEFEATED"),pakkun("PRESENT")],objects:[]};
  if(result.loss)return{mode:"consequence",location:"KONOHA ALLEY · NIGHT",objective:"Return and report.",actors:[kakashi("DEFEATED · WITHDRAWING",true),amt("PRESENT"),pakkun("PRESENT")],objects:[]};
  return{mode:"consequence",location:"KONOHA ALLEY · NIGHT",objective:"Resolve the Battle return.",actors:[kakashi("ACTIVE",true),amt("PRESENT"),pakkun("PRESENT")],objects:[]};
 }
 return null;
}
function kakashiBoardResolver({beatId,context,performance}){
  const sequential=sequentialProjection33910(beatId,context,performance);if(sequential)return sequential;
  ...
  if(beatId===DIRECT_PICKPOCKET_RETURN_34710)return directPickpocketReturnProjection33910(performance);
  if([DIRECT_PICKPOCKET_POLICE_34710,DIRECT_PICKPOCKET_ANBU_34710,DIRECT_PICKPOCKET_KILL_34710,DIRECT_PICKPOCKET_RELEASE_34710].includes(beatId))return directPickpocketDispositionProjection33910(beatId,performance);
  ...
  if(beatId==="kak_intercept_disposition_release_35300"){
   ...
   return{mode:"consequence",location:"KONOHA ALLEY · NIGHT",objective:"Return to ANBU.",actors:released?[kakashi("PACKAGE SECURED",true),amt("RELEASED",false,false,"exit"),pakkun("PRESENT")]:[kakashi("PACKAGE SECURED",true),amt("CONTROLLED"),pakkun("PRESENT")],objects:[]};
  }
  if(beatId==="kak_intercept_disposition_kill_35300")return{mode:"consequence",location:"KONOHA ALLEY · NIGHT",objective:"Resolve the original target.",actors:[kakashi("FINAL DECISION",true),amt("DEFEATED",false,false,"fall"),pakkun("PRESENT")],objects:[{label:"PACKAGE",state:"SECURED"}]};
  ...
  if(String(beatId||"").startsWith("kak_move_closer_two_target_")){const pa=context&&context.kakashiMoveCloserFailurePakkunPresent===true?[pakkun("PRESENT")]:[],motion=String(beatId).includes("_kill_")?"fall":String(beatId).includes("_release_")?"exit":null;return{mode:"encounter",location:"SAKURA TREE · MAIN STREET",objective:"Resolve the two-target disposition.",actors:[kakashi("ACTIVE",true),amt(motion==="exit"?"RELEASED":"DEFEATED",false,false,motion),smuggler(motion==="exit"?"RELEASED":"DEFEATED",false,false,motion),...pa],objects:[{label:"PACKAGE",state:"SECURED BY KAKASHI"}]};}
  ...
  if(String(beatId||"").startsWith("kak_move_closer_failure_ps_")){const motion=String(beatId).includes("_kill_")?"fall":String(beatId).includes("_release_")?"exit":null;return{mode:"encounter",location:"KONOHA ALLEY · NIGHT",objective:"Resolve the Package Smuggler confrontation.",actors:[kakashi("ACTIVE",true),smuggler(motion==="exit"?"RELEASED":"PRESENT",false,false,motion)],objects:[]};}
  ...
  if(beatId==="kak_konoha_direct_strike_kill_all_35910")return{mode:"encounter",location:"SAKURA TREE · MAIN STREET",objective:"Resolve what happens to the three defeated shinobi.",actors:[kakashi("ACTIVE",true),amt("DEFEATED",false,false,"fall"),smuggler("DEFEATED",false,false,"fall"),interceptor("DEFEATED",false,false,"fall")],objects:[{label:"PACKAGE",state:"SECURED BY KAKASHI"}]};
  if(beatId==="kak_konoha_direct_strike_release_all_35910")return{mode:"encounter",location:"SAKURA TREE · MAIN STREET",objective:"Resolve what happens to the three defeated shinobi.",actors:[kakashi("ACTIVE",true),amt("RELEASED",false,false,"exit"),smuggler("RELEASED",false,false,"exit"),interceptor("RELEASED",false,false,"exit")],objects:[{label:"PACKAGE",state:"SECURED BY KAKASHI"}]};
  if(beatId==="kak_konoha_direct_strike_group_disposition_35910"||beatId==="kak_konoha_direct_strike_police_escort_35910"||beatId==="kak_konoha_direct_strike_anbu_escort_35910")return{mode:"encounter",location:"SAKURA TREE · MAIN STREET",objective:"Resolve what happens to the three defeated shinobi.",actors:[kakashi("ACTIVE",true),amt("DEFEATED"),smuggler("DEFEATED"),interceptor("DEFEATED")],objects:[{label:"PACKAGE",state:"SECURED BY KAKASHI"}]};
  ...
  if(beatId==="kak_konoha_original_target_kill_35910")return{mode:"encounter",location:"KONOHA ALLEY · NIGHT",objective:"Resolve ANBU Marked Target's disposition.",actors:[kakashi("ACTIVE",true),amt("DEFEATED",false,false,"fall"),pakkun("PRESENT")],objects:[{label:"PACKAGE",state:"MISSING"}]};
  if(beatId==="kak_konoha_original_target_disposition_35910"||beatId==="kak_konoha_original_target_restrain_35910"||beatId==="kak_konoha_original_target_anbu_escort_35910"||beatId==="kak_konoha_original_target_police_escort_35910")return{mode:"encounter",location:"KONOHA ALLEY · NIGHT",objective:"Resolve ANBU Marked Target's disposition.",actors:[kakashi("ACTIVE",true),amt("DEFEATED"),pakkun("PRESENT")],objects:[{label:"PACKAGE",state:"MISSING"}]};
  ...
  if(["kak_terminal_minato_private_evaluation_35100",TERMINAL_RECEIPT_35100,TERMINAL_FINAL_35100].includes(beatId))return terminalHokageProjection33910(beatId);
  return null;
}
...
registerStorySceneBoardDefinition(scene,{benchmark:true,resolve:kakashiBoardResolver,performanceSequences:{kak_original_rooftop:rooftopPerformance,kak_original_tail:tailPerformance,kak_original_transfer:({context})=>context.kakashiOriginalAction==="observe"?observeTransferPerformance:null,[DIRECT_PICKPOCKET_RETURN_34710]:({beat})=>textPerformance33910(beat,"direct_pickpocket_return"),[DIRECT_PICKPOCKET_POLICE_34710]:({beat})=>textPerformance33910(beat,"direct_pickpocket_police"),[DIRECT_PICKPOCKET_ANBU_34710]:({beat})=>textPerformance33910(beat,"direct_pickpocket_anbu"),[DIRECT_PICKPOCKET_KILL_34710]:({beat})=>textPerformance33910(beat,"direct_pickpocket_kill"),[DIRECT_PICKPOCKET_RELEASE_34710]:({beat})=>textPerformance33910(beat,"direct_pickpocket_release"),kak_seq_debrief_pending:()=>terminalReportOpeningPerformance33910(),kak_terminal_debrief_summary_35100:()=>terminalReportBodyPerformance33910(),kak_terminal_minato_private_evaluation_35100:()=>terminalMinatoPerformance33910(),[TERMINAL_RECEIPT_35100]:()=>terminalReceiptPerformance33910(),[TERMINAL_FINAL_35100]:()=>terminalFinalPerformance33910()},performanceTransitions:{}});
...
function speakerActorId(name){const key=String(name||"").trim().toUpperCase();if(key==="KAKASHI"||key==="KAKASHI HATAKE")return"academy_kakashi";if(key==="ANBU OPERATIVE")return"konoha_anbu_contact";if(key==="ANBU MARKED TARGET")return"anbu_marked_target";if(key==="PACKAGE SMUGGLER")return"package_smuggler";if(key==="MASKED INTERCEPTOR")return"masked_interceptor";if(key==="MINATO"||key==="MINATO NAMIKAZE")return"hokage_minato";if(key==="PAKKUN"||key==="NINKEN")return"pakkun";return null;}
...
function makeNarrationPanel(cue){const panel=document.createElement("section"),kind=String(cue&&cue.kind||"narration").toLowerCase();panel.className="sc-narration-panel-33910"+(kind==="record"?" is-record":"");const kicker=document.createElement("div");kicker.className="sc-narration-kicker-33910";kicker.textContent=kind==="record"?"CHRONICLE RECEIPT":`${kind.toUpperCase()} · ACADEMY KAKASHI`;const copy=document.createElement("div");copy.className="sc-narration-copy-33910";copy.textContent=escapeText(cue&&cue.text||"");panel.append(kicker,copy,makeAdvanceButton());return panel;}
...
function intentMeta(choice){const label=String(choice&&choice.label||"").toLowerCase(),id=String(choice&&choice.choiceId||"").toLowerCase(),text=label+" "+id;if(label.includes("return to anbu")||label.includes("report"))return{intent:"RETURN",glyph:"↩"};if(label.includes("continue"))return{intent:"CONTINUE",glyph:"›"};if(label.includes("take them to")||label.includes("take him to")||label.includes("transfer"))return{intent:"TRANSFER",glyph:"⇢"};if(label.includes("observe")||label.includes("watch"))return{intent:"OBSERVE",glyph:"◎"};if(label.includes("attack")||label.includes("fight")||label.includes("strike")||label.includes("kill")||label.includes("take him down"))return{intent:"COMBAT",glyph:"✦"};if(label.includes("package")||label.includes("secure")||label.includes("pickpocket")||label.includes("demand"))return{intent:"OBJECTIVE",glyph:"◇"};if(label.includes("closer")||label.includes("pursue")||label.includes("chase")||label.includes("stay"))return{intent:"POSITION",glyph:"↗"};if(label.includes("ask")||label.includes("where"))return{intent:"INTEL",glyph:"?"};if(text.includes("police")||text.includes("anbu"))return{intent:"TRANSFER",glyph:"⇢"};return{intent:"TACTICAL",glyph:"›"};}
...
function bindStageAdvance33910(stage,layer){
  if(!stage||typeof stage.addEventListener!=="function"||boundStages33910.has(stage))return false;
  boundStages33910.add(stage);
  stage.addEventListener("click",event=>{
    if(!isActiveKakashi33910()||event&&event.defaultPrevented)return;
    const mode=layer&&layer.dataset?String(layer.dataset.scBoardUiMode||""):"";
    if(mode!=="dialogue"&&mode!=="performance_narration")return;
    if(interactiveTarget33910(event&&event.target))return;
...
function syncPresentation(){
  ...
  if(!runtime||runtime.sceneId!==scene){
    delete layer.dataset.scBoardUiMode;
    clearPerformanceSurface(stage);
    syncNativePerformanceLayout33910(layer,false);
    return false;
  }
  bindStageAdvance33910(stage,layer);
  ...
  let mode="narration";
  if(performance&&performance.cue)mode=isDialogueCue33910(performance.cue)?"dialogue":"performance_narration";
  else if(beat&&beat.mode==="choice")mode="decision";
  else if(beat&&beat.mode==="battle_transition")mode="battle_transition";
  const performed=mode==="dialogue"||mode==="performance_narration";
  layer.dataset.scBoardUiMode=mode;
  const office=!!beat&&["kak_terminal_minato_private_evaluation_35100",TERMINAL_RECEIPT_35100,TERMINAL_FINAL_35100].includes(beat.beatId),receipt=!!beat&&beat.beatId===TERMINAL_RECEIPT_35100;
  const cursorState=runtime&&runtime.localContext&&runtime.localContext.__storyPerformanceCursor33900,choicePerformed=!!(beat&&beat.mode==="choice"&&cursorState&&cursorState.beatId===beat.beatId&&cursorState.complete===true);
  if(layer.dataset){layer.dataset.scTerminalOffice=office?"true":"false";layer.dataset.scTerminalReceipt=receipt?"true":"false";layer.dataset.scChoicePerformanceComplete=choicePerformed?"true":"false";}
...
const PRE_RENDER_33910=typeof globalThis.renderStoryScenePresentationLayer==="function"?globalThis.renderStoryScenePresentationLayer:null;let lastRenderedSceneId=currentRuntime()&&currentRuntime().sceneId||null;let renderOwnerSource33910="";
if(PRE_RENDER_33910){function renderStoryScenePresentationLayer33910(){const before=currentRuntime(),entering=!!before&&before.sceneId===scene&&lastRenderedSceneId!==scene,curtain=entering?createEntryCurtain():null,result=PRE_RENDER_33910.apply(this,arguments),after=currentRuntime();lastRenderedSceneId=after&&after.sceneId||null;syncPresentation();if(curtain)revealEntryCurtain(curtain);return result;}renderOwnerSource33910=renderStoryScenePresentationLayer33910.toString();globalThis.renderStoryScenePresentationLayer=renderStoryScenePresentationLayer33910;try{renderStoryScenePresentationLayer=renderStoryScenePresentationLayer33910;}catch(_error){}}
const PRESENTATION_HOOK_ID_33910="kakashi_scene_board_projection_33910";
if(typeof globalThis.registerStorySceneBoardRenderHook==="function"){
  globalThis.registerStorySceneBoardRenderHook(PRESENTATION_HOOK_ID_33910,()=>{syncPresentation();return false;});
}
...
function runKakashiSceneBoardPolish33910Diagnostics(){
  ...
  const sequentialIds=["kak_observe_secure_package_battle","kak_observe_secure_package_return","kak_observe_secure_package_pakkun_arrival","kak_observe_secure_package_amt_battle","kak_observe_secure_package_amt_return","kak_seq_mi_battle","kak_seq_mi_return","kak_seq_ps_battle","kak_seq_ps_return","kak_seq_pakkun_arrival","kak_seq_amt_battle","kak_seq_amt_return"];
  const checks={patchId:PATCH_ID==="kakashi_scene_board_model_v22_33910_2026_09_22",singleExplicitPresentationOwner:!installStyle.toString().includes("sc-kakashi-wipe-33910")&&!String(globalThis.performKakashiSceneWipe33910||"")&&!syncPresentation.toString().includes("renderStorySceneBoard33900"),sequentialBattleReturnProjectionComplete:sequentialIds.every(id=>{const q=sequentialProjection33910(id,{battleResume:{authored:{resultState:"player_side_victory",playerActionOpportunityCount:2}}},null);return!!(q&&Array.isArray(q.actors)&&q.actors.length>0&&q.objective!==undefined);})&&["kak_observe_secure_package_battle","kak_observe_secure_package_amt_battle","kak_seq_mi_battle","kak_seq_ps_battle","kak_seq_amt_battle"].every(id=>sequentialProjection33910(id,{},null).mode==="battle_transition"),compatibilityFoldedIntoCanonicalOwner:installStyle.toString().includes("former 33920 compatibility behavior")&&bindStageAdvance33910.toString().includes("advanceStoryScene"),nativePerformanceLayoutSingleOwned:syncNativePerformanceLayout33910.toString().includes('removeProperty("display")')&&!syncNativePerformanceLayout33910.toString().includes('setProperty("display","none","important")')&&syncPresentation.toString().includes("syncNativePerformanceLayout33910"),boardRenderLifecycleSync:typeof globalThis.registerStorySceneBoardRenderHook==="function"&&PRESENTATION_HOOK_ID_33910==="kakashi_scene_board_projection_33910",activeSceneGuardedAdvance:bindStageAdvance33910.toString().includes("isActiveKakashi33910"),inactiveSceneClearsPerformance:syncPresentation.toString().includes("clearPerformanceSurface(stage)")&&syncPresentation.toString().includes("syncNativePerformanceLayout33910(layer,false)"),synchronousFirstPaintPresentation:renderOwnerSource33910.includes("syncPresentation()")&&!renderOwnerSource33910.includes("scheduleSync()"),singleDialogueLane:installStyle.toString().includes("top:4%!important")&&!installStyle.toString().includes("top:27%!important")&&!installStyle.toString().includes("top:"+"2%!important")&&!installStyle.toString().includes("data-sc-postmi-35830")&&!installStyle.toString().includes("right:6%!important"),receiptPreservesLineBreaks:installStyle.toString().includes("white-space:pre-line!important"),multiActorKakashiLayouts:installStyle.toString().includes('data-count="6"')&&PAKKUN_IMAGE==="Assets/Summons/pakkun.png"&&kakashiBoardResolver({beatId:"kak_konoha_group_anbu_handoff_35920",context:{},performance:null}).actors.length>=3,stableActorSafeZone:installStyle.toString().includes("bottom:38%!important")&&installStyle.toString().includes("bottom:-14px!important"),secondaryActorsRemainReadable:installStyle.toString().includes("opacity:.78;")&&installStyle.toString().includes("brightness(.90)")&&!installStyle.toString().includes("opacity:.78!important"),battleTransitionSharesBoardGeometry:installStyle.toString().includes('data-sc-board-ui-mode="battle_transition"')&&syncPresentation.toString().includes('beat.mode==="battle_transition"'),
    ...
    terminalReportBodyUsesDynamicPerformance:terminalReportBodyPerformance33910.toString().includes("terminalReportSplit33910")&&terminalReportPerformance33910.toString().includes("SC_ALPHA_KAKASHI_DYNAMIC_TERMINAL_35940"),directPickpocketReturnUsesPreChoicePerformance:authoredBeatText33910.toString().includes("presentationResolver")&&textPerformance33910.toString().includes("split(/\\n{2,}/)")&&directPickpocketReturnProjection33910.toString().includes("ESCAPING WITH PACKAGE"),directPickpocketDispositionPresentationComplete:[DIRECT_PICKPOCKET_POLICE_34710,DIRECT_PICKPOCKET_ANBU_34710,DIRECT_PICKPOCKET_KILL_34710,DIRECT_PICKPOCKET_RELEASE_34710].every(id=>!!directPickpocketDispositionProjection33910(id,{index:0,cue:{text:"qa"},sequence:[{text:"qa"}]})),receiptUsesPagedRecordPerformance:terminalReceiptPerformance33910.toString().includes("receiptText")&&terminalReceiptPerformance33910.toString().includes('kind:"record"')&&terminalReceiptPerformance33910.toString().includes('blocks.splice(0,2'),terminalFinalObjectiveCleared:terminalHokageProjection33910(TERMINAL_FINAL_35100).objective==="",contextualChoiceIntent:intentMeta({choiceId:DIRECT_PICKPOCKET_DEFEAT_CHOICE_34710,label:"RETURN TO ANBU"}).intent==="RETURN"&&intentMeta({choiceId:"continue",label:"CONTINUE"}).intent==="CONTINUE",officeHierarchyStyled:installStyle.toString().includes('data-sc-terminal-office="true"')&&installStyle.toString().includes('data-actor-id="hokage_minato"'),semanticActorMotionOwned:actor.toString().includes("motion=null")&&kakashiBoardResolver({beatId:"kak_konoha_direct_strike_kill_all_35910",context:{},performance:null}).actors.filter(row=>row.id!=="academy_kakashi").every(row=>row.motion==="fall")&&kakashiBoardResolver({beatId:"kak_konoha_direct_strike_release_all_35910",context:{},performance:null}).actors.filter(row=>row.id!=="academy_kakashi").every(row=>row.motion==="exit"),terminalReportStagesAnbu:(()=>{const q=kakashiBoardResolver({beatId:"kak_seq_debrief_pending",context:{},performance:null});return q&&q.location==="KONOHA ROOFTOP · NIGHT"&&q.actors.some(row=>row.id==="konoha_anbu_contact")&&q.actors.some(row=>row.id==="academy_kakashi");})(),terminalReportOpeningIsClickPerformance:(()=>{const seq=terminalReportOpeningPerformance33910();if(!Array.isArray(seq)||!seq.length)return false;const report=seq[seq.length-1];return report.kind==="dialogue"&&report.text==="Report."&&report.focusActorRef==="konoha_anbu_contact"&&(seq.length===1||(seq[0].kind==="narration"&&seq[0].text==="Kakashi returns to the rooftop."));})(),terminalReportOpeningFocusesAnbu:(()=>{const q=terminalReportProjection33910("kak_seq_debrief_pending",{});return q.actors.some(row=>row.id==="konoha_anbu_contact"&&row.focus===true&&row.state==="SPEAKING")&&!q.actors.some(row=>row.id==="academy_kakashi"&&row.focus===true);})(),terminalHokageSceneStaged:(()=>{const q=kakashiBoardResolver({beatId:"kak_terminal_minato_private_evaluation_35100",context:{},performance:null});return q&&q.location==="HOKAGE ADMINISTRATION · NIGHT"&&q.actors.some(row=>row.id==="hokage_minato")&&q.actors.some(row=>row.id==="konoha_anbu_contact")&&MINATO_IMAGE==="Assets/Kage/kage_minato.png"&&HOKAGE_INTERIOR_PATH==="Kakashi Origin Backdrop/hokage_administration_interior_night.png";})(),
...
globalThis.SC_KAKASHI_SCENE_BOARD_POLISH_33910=Object.freeze({patchId:PATCH_ID,model:"scene_board_v22",scene01Authority:SCENE_01_AUTHORITY,scene02Authority:SCENE_02_AUTHORITY,browserGoldenClaimed:false});
