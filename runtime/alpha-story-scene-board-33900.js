...
function renderStorySceneBoard33900(){
  ...
  rendering=true;try{installStyle();layer.dataset.scSceneBoard="true";layer.dataset.scSceneMode=projection.mode||"conversation";applyBoardBackdrop(stage,runtime);installFallbackBackdropCut33900(layer,runtime,currentBeat(runtime));let board=stage.querySelector?stage.querySelector(".sc-scene-board-33900"):null;if(!board){board=document.createElement("section");board.className="sc-scene-board-33900";board.setAttribute("aria-hidden","true");stage.appendChild(board);}const signature=JSON.stringify(projection);if(board.dataset&&board.dataset.signature!==signature){board.innerHTML=boardMarkup(projection);board.dataset.signature=signature;}updatePerformancePanel(layer,runtime);return true;}finally{rendering=false;}
}
...
sharedCutExported:typeof globalThis.performStorySceneCut33900==="function",sharedBackdropOwnerExported:typeof globalThis.applyStorySceneBoardBackdrop33900==="function",fullScreenBackdropFailsClosed:installStyle.toString().includes('background:#020508!important')&&!installStyle.toString().includes('background:transparent!important')&&applyBoardBackdrop.toString().includes("layer.dataset.scSceneBoardBackdrop")&&applyBoardBackdrop.toString().includes('layer.style.setProperty("--sc-scene-board-backdrop"'),browserGoldenClaimed:false};
