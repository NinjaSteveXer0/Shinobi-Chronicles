// ============================================================================
// ISSUE #188 — ACADEMY KAKASHI DIRECT OPENING CONSUMER — 34710
//
// Scene 02 authority requires all four opening choices to be selectable.
// 34700 already owns the closed factual possibility envelopes for direct Attack
// and direct Pickpocket; this narrow consumer upgrades only those two envelopes
// with authoritative Origin-ledger commit ownership, installs proof-bearing
// Story consequences, and releases them through the existing 34200 guard.
//
// Scene 03C / 03D player-facing prose is intentionally NOT authored here.
// Both routes land on explicit non-legacy authoring boundaries after their
// semantic/factual state is committed. Direct Pickpocket failure may enter its
// already-authorised exact 3-v-1 Battle seam. No route may fall through the
// obsolete kak_original_transfer beat.
// ============================================================================
...
const PATCH_ID="alpha_kakashi_direct_opening_consumer_34710_2026_09_17";
...
const PICKPOCKET_SUCCESS_BOUNDARY="kak_scene03d_pickpocket_success_authoring_boundary_34710";
...
const PICKPOCKET_OUTCOMES=Object.freeze([
  ...
  outcome("PICKPOCKET_DIRECT_FAILURE_DETECTED_3V1",{outcomeClass:"PICKPOCKET_DIRECT_FAILURE_DETECTED_3V1",cleanExtractionSucceeded:false,maskedInterceptorVisible:true,pakkunPresent:false,battleRequired:true,battleConfigId:PICKPOCKET_3V1_CONFIG},"battle_transition")
]);
...
function commitDirect({receipt,request,result},bindingRef){
  ...
  const packageState=isAttack
    ...
    :{objectRef:WORLD_OBJECT_REF,currentHolderClass:pickSuccess?"KAKASHI":null,custodyClass:pickSuccess?"KAKASHI":"UNRESOLVED",handoffCompleted:false};
  const fact={
    ...
    worldFacts:{handoffInterrupted:isAttack?result&&result.handoffInterrupted===true:false,cleanExtractionSucceeded:pickSuccess,withdrawalCompletedUndetected:pickSuccess,maskedInterceptorVisible:pickFailure,pakkunPresent:false,battleRequired:result&&result.battleRequired===true,battleConfigId:result&&result.battleConfigId||null},
  ...
  if(existing){const ef=factOf(existing);if(String(existing.storySceneInstanceId||ef.storySceneInstanceId||"")!==requestedInstance||String(ef.selectedOutcomeRef||"")!==selected)return{success:false,reason:"kakashi_direct_opening_occurrence_replay_mismatch"};}
    ...
    const participants=isPick&&pickFailure?[ORIGIN_ID,"academy_kakashi_origin_amt","academy_kakashi_origin_package_smuggler","academy_kakashi_origin_masked_interceptor"]:[ORIGIN_ID];
...
function upgradeBindings(){
  ...
  const attack=PROVIDER.registerStoryFactualResolverBinding(ATTACK_BINDING,{...base,outcomes:ATTACK_OUTCOMES,commitResult:commitAttack,metadata:{storyUnitRef:ORIGIN_ID,selectionOnly:false,authoritativeCommitOwnerRef:PATCH_ID,anchorRefs:["AK_SA_003"],maskedInterceptorNotImplied:true}});
...
function installBeatsAndChoices(){
  ...
  const beats=[
    ...
    {beatId:PICKPOCKET_SUCCESS_BOUNDARY,mode:"narration",environmentRef:action.environmentRef||SAKURA,text:"",exitScene:false,allowPresentationClose:false},
    ...
    {beatId:PICKPOCKET_BATTLE_RETURN,mode:"narration",environmentRef:action.environmentRef||SAKURA,text:"",exitScene:false,allowPresentationClose:false}
  ];
  ...
  pick.nextBeatId=PICKPOCKET_SUCCESS_BOUNDARY;pick.consequenceRequests=[{requestId:PICKPOCKET_REQUEST,kind:"domain",resolve:()=>resolveDirectChoice(pick,PICKPOCKET_CHOICE,PICKPOCKET_BINDING)}];
...
function diagnostics(){
  const def=scene(),action=def&&def.beatMap instanceof Map?def.beatMap.get(ACTION_BEAT):null;
