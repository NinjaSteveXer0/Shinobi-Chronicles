...
addBeat("v2_mi_restrained_next",{mode:"choice",backdrop:B.fight,location:"SAKURA TREE · NIGHT",objective:"Retrieve the package.",actors:["kakashi","mi"],preset:"post_battle",cues:[N("Kakashi looks once toward the route the package took, then back to Masked Interceptor."),N("If he follows, he has to leave her here. Leaving her free is not an option."),N("He draws ninja wire."),Q("MASKED INTERCEPTOR","So you're still going after them."),Q("KAKASHI","Yes."),N("Kakashi binds her wrists and fixes the restraint to the Sakura tree."),N("She tests the wire once, then stops."),Q("MASKED INTERCEPTOR","You're leaving me tied to a tree."),Q("KAKASHI","Until I come back."),Q("MASKED INTERCEPTOR","Confident."),N("Kakashi is already looking toward the rooftops."),Q("KAKASHI","Wait and see."),N("He leaves her beneath the Sakura tree.")],choices:[
 C("restrained_mi_ps","GO AFTER PACKAGE SMUGGLER","v2_ps_pursuit_resolver",{patch:()=>history("PURSUE_PS_AFTER_RESTRAIN_MI")}),
]});

// PS pursuit after MI.
addBeat("v2_ps_pursuit_resolver",{mode:"choice",backdrop:B.sakura,location:"KONOHA · PURSUIT",objective:"Recover the package.",actors:["kakashi","ps"],preset:"pursuit",onEnter:()=>resolverEnter("psPursuit"),cues:[N("Kakashi moves before Package Smuggler can disappear completely."),N("The Sakura tree drops behind him as the chase cuts through a narrow street and up across the roofs."),N("The receiver already has a lead, but Kakashi can still see the package tucked tight against his side."),N("Package Smuggler is not searching for a hiding place. He is trying to build enough distance that he will never need one."),N("Kakashi increases his pace.")],choices:[
...
addBeat("v2_stay_package_pursuit_resolver",{mode:"choice",backdrop:B.sakura,location:"SAKURA TREE · PURSUIT",objective:"Catch ANBU Marked Target.",actors:["kakashi","amt"],preset:"pursuit",onEnter:()=>resolverEnter("stayPackagePursuit"),cues:[N("ANBU Marked Target is already moving away with the package. Kakashi goes after him."),N("Package Smuggler catches the movement at the alley mouth."),Q("PACKAGE SMUGGLER","There!"),N("The carrier looks back and sees Kakashi for the first time."),Q("ANBU MARKED TARGET","A kid?"),N("Kakashi runs straight past Package Smuggler's reach."),Q("PACKAGE SMUGGLER","Don't stop!"),Q("ANBU MARKED TARGET","You said I was done!"),Q("PACKAGE SMUGGLER","Not while you're holding it!"),N("That gives Kakashi one more useful fact: the carrier was never meant to keep the package."),N("The chase breaks into the open around the Sakura tree."),Q("ANBU MARKED TARGET","What do you want?"),Q("KAKASHI","The package."),N("The man gives a short, breathless laugh and changes direction around the tree."),N("Kakashi stops trying to match his speed and starts reading the exits instead.")],choices:[
...
addBeat("v2_stay_package_intercept",{mode:"choice",backdrop:B.intercept,location:"KONOHA ALLEYWAY · NIGHT",objective:"Recover the package.",actors:["kakashi","amt","pakkun"],preset:"intercept",onEnter:()=>setPakkun(true),cues:[N("Kakashi cuts inside the target's route."),N("ANBU Marked Target reaches the far side of the alley and stops."),N("A small ninken is standing in the exit."),Q("ANBU MARKED TARGET","Seriously?"),N("Kakashi arrives behind him."),Q("PAKKUN","This yours?"),Q("KAKASHI","Apparently."),Q("KAKASHI","Thanks."),Q("ANBU MARKED TARGET","You've been chasing this thing all night."),Q("KAKASHI","You've been running with it."),N("The target glances between Kakashi and the ninken."),Q("ANBU MARKED TARGET","And now?"),N("Kakashi's eye settles on the package."),Q("KAKASHI","Now you stop.")],choices:[
...
addBeat("v2_ask_where",{mode:"choice",backdrop:B.intercept,location:"KONOHA ALLEYWAY · NIGHT",objective:"Recover the package.",actors:["kakashi","amt","pakkun"],preset:"intercept",onEnter:()=>mutate(s=>{s.knowledge.askWhere=true;s.knowledge.downstreamDestinationKnown=false;}),cues:[N("ANBU Marked Target keeps one hand close to the package while Pakkun holds the escape line."),N("He is waiting for Kakashi to demand it."),Q("KAKASHI","Where was it going?"),N("The question catches him off guard."),Q("ANBU MARKED TARGET","You think they told me?"),Q("KAKASHI","You asked."),N("The man gives him a tired look."),Q("ANBU MARKED TARGET","I carry it to him. He carries it somewhere else."),Q("KAKASHI","Where?"),Q("ANBU MARKED TARGET","I asked the same thing."),N("Kakashi remembers Package Smuggler's answer: away from you."),Q("KAKASHI","He didn't tell you."),Q("ANBU MARKED TARGET","Now you're caught up."),N("Pakkun looks between them."),Q("PAKKUN","Efficient."),N("Neither man laughs.")],choices:[
...
addBeat("v2_take_down_loss",{backdrop:B.intercept,location:"KONOHA ALLEYWAY · NIGHT",objective:"Return to ANBU.",actors:["kakashi","pakkun"],preset:"post_battle",onEnter:ctx=>captureBattle("take_down_amt",ctx,s=>{s.participants.AMT.state="ESCAPED";s.package.holder="KAKASHI";s.package.recovered=true;s.package.neutral=false;}),cues:[N("ANBU Marked Target wins the fight and uses the opening to escape."),N("The package is still on the ground when he leaves."),N("Pakkun gets between the target and the objective long enough for Kakashi to recover it."),N("By the time Kakashi is back on his feet with the package secured, the man is gone.")],nextBeatId:"v2_report"});
...
addBeat("v2_ps_missing_loss",{backdrop:B.fight,location:"KONOHA · NIGHT",objective:"Return to ANBU.",actors:["kakashi"],preset:"post_battle",onEnter:ctx=>captureBattle("ps_direct",ctx,s=>{s.participants.PS.state="ESCAPED";}),cues:[N("Package Smuggler wins the fight and disappears before Kakashi can recover."),N("ANBU Marked Target is already gone with the package."),N("Kakashi is left with neither the objective nor a prisoner.")],nextBeatId:"v2_report"});
...
addBeat("v2_cutoff_setup",{backdrop:B.sakura,location:"SAKURA TREE · NIGHT",objective:"Stop the package.",actors:["kakashi","amt","ps","pakkun"],preset:"sakura_group",onEnter:()=>setPakkun(true),cues:[N("Kakashi does not follow either man. He cuts across both routes toward the Sakura tree."),N("ANBU Marked Target sees what he is doing first."),Q("PACKAGE SMUGGLER","Don't let him get in front of you!"),N("Too late."),N("Kakashi reaches the remaining exit line."),N("ANBU Marked Target turns and finds a small ninken already standing in the street ahead."),Q("ANBU MARKED TARGET","Oh, come on."),N("Pakkun looks past him toward Kakashi."),Q("PAKKUN","This yours?"),Q("KAKASHI","Apparently."),N("Package Smuggler reaches the choke point behind them."),N("No one has a clean way out.")],nextBeatId:"v2_battle_cutoff"});
...
addBeat("v2_improved_pickpocket_resolver",{mode:"choice",backdrop:B.endAlley,location:"END OF ALLEYWAY · NIGHT",objective:"Take the package cleanly.",actors:["kakashi","amt","ps"],preset:"observe",onEnter:()=>resolverEnter("improvedPickpocket"),cues:[N("Kakashi has already shortened the distance and heard the contingency."),N("ANBU Marked Target's hand is still over the package. Package Smuggler is waiting."),N("Kakashi uses the closer position to try to remove the package before the handoff can finish.")],choices:[
...
addBeat("v2_improved_2v1_loss",{backdrop:B.fight,location:"SAKURA TREE · NIGHT",objective:"Return to ANBU.",actors:["kakashi"],preset:"post_battle",onEnter:ctx=>captureBattle("improved_2v1",ctx,s=>{s.participants.AMT.state="ESCAPED";s.participants.PS.state="ESCAPED";s.package.holder="AMT";}),cues:[N("ANBU Marked Target and Package Smuggler break away after the fight."),N("The original carrier keeps the package."),N("The handoff never happens, and the masked shinobi never appears.")],nextBeatId:"v2_report"});
