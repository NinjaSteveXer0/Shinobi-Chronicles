...
function diagnostics(){
 ...
 const checks={
  ...
  resolverPseudoChoicesRetired:!JSON.stringify(beats).includes('"CONTINUE"')&&beats.filter(b=>b.machineResolved===true).length===10&&beats.filter(b=>b.machineResolved===true).every(b=>b.mode==="resolver"&&b.choices.length===2&&b.choices.every(ch=>ch.label==="RESOLVE RESULT")),
