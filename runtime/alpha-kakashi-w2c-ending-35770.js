...
const PATCH_ID="alpha_kakashi_w2c_ending_35770_v10_2026_09_22";
...
function installStyle(){
  ...
  "."+BOARD_CLASS+"[data-w2c-stage=\"office\"] .sc-scene-board-33900__actor{position:absolute!important;width:min(11.5vw,158px)!important;max-height:286px!important}"+
  "."+BOARD_CLASS+"[data-w2c-stage=\"office\"] .sc-scene-board-33900__actor[data-actor-id=\""+MINATO+"\"]{left:41%!important;top:1%!important;width:min(18vw,252px)!important;max-height:430px!important;z-index:6}"+
  "."+BOARD_CLASS+"[data-w2c-stage=\"office\"] .sc-scene-board-33900__actor[data-actor-id=\""+AMT+"\"]{left:28%!important;bottom:23.5%!important;z-index:3}"+
  "."+BOARD_CLASS+"[data-w2c-stage=\"office\"] .sc-scene-board-33900__actor[data-actor-id=\""+ANBU+"\"]{left:54%!important;bottom:23.5%!important;z-index:5}"+
  "."+BOARD_CLASS+"[data-w2c-stage=\"office\"] .sc-scene-board-33900__actor[data-actor-id=\""+PS+"\"]{left:67%!important;bottom:23.5%!important;z-index:3}"+
  "."+BOARD_CLASS+"[data-w2c-stage=\"office\"] .sc-scene-board-33900__objects{left:3.5%!important;right:auto!important;top:10%!important;bottom:auto!important;width:max-content!important;max-width:min(26%,340px)!important;height:auto!important;min-height:0!important;max-height:none!important}"+
  "."+BOARD_CLASS+" .sc-w2c-office-anbu-enter-35770{animation:w2cOfficeAnbuEnter35770 520ms cubic-bezier(.18,.76,.24,1) both!important}"+
  "@keyframes w2cOfficeAnbuEnter35770{0%{opacity:0;transform:translateX(-16vw) scale(.94);filter:brightness(.45)}100%{opacity:.72;transform:translateX(0) scale(.95);filter:saturate(.78) brightness(.88)}}"+
  "."+BOARD_CLASS+" .sc-w2c-office-anbu-enter-35770.is-focus{animation-name:w2cOfficeAnbuEnterFocus35770}@keyframes w2cOfficeAnbuEnterFocus35770{0%{opacity:0;transform:translateX(-16vw) scale(.96)}100%{opacity:1;transform:translateX(0) scale(1)}}"+
  ...
  ".sc-kakashi-kill-35770{position:fixed;z-index:2147483400;pointer-events:none;overflow:hidden;isolation:isolate;background:transparent}.sc-kakashi-kill-35770 img{position:absolute;bottom:13%;height:min(58vh,510px);filter:drop-shadow(0 18px 24px #000)}"+
  ".sc-kakashi-kill-35770 .kak{left:12%}.sc-kakashi-kill-35770 .mi{right:12%}.sc-kakashi-kill-35770 .slash{position:absolute;left:-12%;top:47%;width:130%;height:12%;background:#000;box-shadow:0 -2px 0 rgba(255,255,255,.72),0 3px 0 rgba(120,0,0,.58);transform:rotate(-17deg) scaleX(0);transform-origin:left center}.sc-kakashi-kill-35770 .flash{position:absolute;inset:0;background:#fff;opacity:0}.sc-kakashi-kill-35770 .black{position:absolute;inset:0;background:#000;opacity:0}"+
  ".sc-kakashi-kill-35770.is-active .kak{animation:kak35770 1050ms cubic-bezier(.18,.75,.2,1) forwards!important}.sc-kakashi-kill-35770.is-active .mi{animation:mi35770 1050ms ease-out forwards!important}.sc-kakashi-kill-35770.is-active .slash{animation:slash35770 1050ms forwards!important}.sc-kakashi-kill-35770.is-active .flash{animation:flash35770 1050ms forwards!important}.sc-kakashi-kill-35770.is-active .black{animation:black35770 1050ms forwards!important}"+
...
function card(id,label,img,state,focus,extra=""){return '<figure class="sc-scene-board-33900__actor '+(focus?"is-focus ":"")+(extra||"")+'" data-actor-id="'+esc(id)+'"><div class="sc-scene-board-33900__actor-frame"></div><img src="'+esc(img)+'" alt=""><figcaption class="sc-scene-board-33900__actor-tag"><strong>'+esc(label)+'</strong><small>'+esc(state)+'</small></figcaption></figure>';}
function board(rt,p){
  ...
  const anbuExtra=p&&p.index===0?"sc-w2c-office-anbu-enter-35770":"";
  return '<div class="sc-scene-board-33900__top"><div class="sc-scene-board-33900__location">HOKAGE ADMINISTRATION · NIGHT</div></div><div class="sc-scene-board-33900__actors" data-count="4">'+card(MINATO,"MINATO","Assets/Kage/kage_minato.png","HOKAGE",f===MINATO)+card(AMT,"ANBU MARKED TARGET","NPC/anbu_marked_target.png","ALIVE",f===AMT)+card(ANBU,"ANBU OPERATIVE","NPC/konoha_anbu.png","PRESENT",f===ANBU,anbuExtra)+card(PS,"PACKAGE SMUGGLER","NPC/package_smuggler.png","ALIVE",f===PS)+'</div><div class="sc-scene-board-33900__objects sc-live-state-callouts-33900"><span class="sc-scene-board-33900__object sc-live-state-callout-33900 is-committed"><b>PACKAGE</b>RECOVERED · HIDDEN OPERATION</span></div>';
}
function applyCanonicalBackdrop35770(stage,rt){return !!(stage&&typeof globalThis.applyStorySceneBoardBackdrop33900==="function"&&globalThis.applyStorySceneBoardBackdrop33900(stage,rt));}
function render(){
  ...
  applyCanonicalBackdrop35770(stage,rt);
  ...
  // Canonical 33910 projects the active performance cue into dialogue/narration UI.
  return true;
}
...
function playLethalAttemptAnimation35770(onComplete){
  ...
  const layer=document.getElementById("story-scene-presentation-layer");if(!layer||!document.body)return false;
  const stage=(layer.querySelector&&layer.querySelector(".sc-chronicle-stage"))||layer;installStyle();animationRunning=true;
  for(const old of Array.from(document.querySelectorAll?document.querySelectorAll(".sc-kakashi-kill-35770"):[]))try{old.remove();}catch(_error){}
  const rect=typeof layer.getBoundingClientRect==="function"?layer.getBoundingClientRect():null;
  const n=document.createElement("div");n.className="sc-kakashi-kill-35770";
  n.style.left=(rect&&Number.isFinite(rect.left)?rect.left:0)+"px";
  n.style.top=(rect&&Number.isFinite(rect.top)?rect.top:0)+"px";
  n.style.width=(rect&&rect.width?rect.width:(document.documentElement&&document.documentElement.clientWidth||window.innerWidth||0))+"px";
  n.style.height=(rect&&rect.height?rect.height:(document.documentElement&&document.documentElement.clientHeight||window.innerHeight||0))+"px";
  n.dataset.scLethalAttempt="true";
  n.innerHTML='<img class="kak" src="Assets/Academy Student/academy_kakashi.png" alt=""><img class="mi" src="NPC/masked_interceptor.png" alt=""><div class="flash"></div><div class="slash"></div><div class="black"></div>';
  document.body.appendChild(n);
  const start=function(){if(!n.isConnected)return;n.classList.add("is-active");};
  if(typeof requestAnimationFrame==="function")requestAnimationFrame(function(){requestAnimationFrame(start);});else setTimeout(start,16);
  setTimeout(function(){
    try{n.remove();}catch(_error){}
    animationRunning=false;
    if(typeof onComplete==="function")onComplete(stage);
  },1140);
  return true;
}
function beginConfirmedKill(){
  const rt=active();if(!rt||rt.beatId!==SOURCE_HOLD||!confirmedKill()||rt.localContext&&rt.localContext.kakashiScene06AW2CPursuitAvailable===true)return false;
  removeQaOutcome();
...
function sharedSceneCut35770(next){
  if(typeof globalThis.performStorySceneCut33900==="function")return globalThis.performStorySceneCut33900(next);
  return typeof next==="function"?next():{success:false,reason:"w2c_scene_cut_continuation_missing"};
}
...
function hooks(){
  ...
  globalThis.advanceStoryScene=function(choiceId=null){
    const rt=active();
    if(rt&&rt.beatId===SOURCE_HOLD&&String(rt.localContext&&rt.localContext.kakashiScene06AW2COutcomeRef||"")==="LETHAL_ATTEMPT_KILLED"&&!(rt.localContext&&rt.localContext.kakashiScene06AW2CPursuitAvailable===true)){beginConfirmedKill();return{success:true,type:"confirmed_kill_presentation_started"};}
    if(rt&&[AFTERMATH,SCENE07,SCENE08].includes(rt.beatId)&&choiceId==null){const p=sequence(rt);if(!p)return{success:false,reason:"w2c_sequence_missing"};if(!p.atEnd){const key=rt.beatId===AFTERMATH?CURSOR_A:rt.beatId===SCENE07?CURSOR_7:CURSOR_8;rt.localContext={...(rt.localContext||{}),[key]:p.index+1};save();try{renderStoryScenePresentationLayer();}catch(_error){}return{success:true,beatId:rt.beatId,cueIndex:p.index+1};}if(rt.beatId===AFTERMATH)return sharedSceneCut35770(enter07);if(rt.beatId===SCENE07)return sharedSceneCut35770(enter08);return sharedSceneCut35770(openReceipt);}
    const delegated=PA.apply(this,arguments);
    const after=active();
    if(after&&after.beatId===SOURCE_HOLD&&String(after.localContext&&after.localContext.kakashiScene06AW2COutcomeRef||"")==="LETHAL_ATTEMPT_KILLED"&&!(after.localContext&&after.localContext.kakashiScene06AW2CPursuitAvailable===true)){
  ...
  if(typeof globalThis.registerStorySceneBoardRenderHook==="function")globalThis.registerStorySceneBoardRenderHook("kakashi_w2c_kill_35770",()=>render());
...
function installOutcomeWatchdog(){
  ...
  watchdogId=setInterval(function(){
    ...
    if(rt.beatId===SOURCE_HOLD){
      const selected=String(rt.localContext&&rt.localContext.kakashiScene06AW2COutcomeRef||"");
      if(selected==="LETHAL_ATTEMPT_KILLED"&&!(rt.localContext&&rt.localContext.kakashiScene06AW2CPursuitAvailable===true)){removeQaOutcome();beginConfirmedKill();}
      else{
...
function diagnostics(){const d=scene(),m=d&&d.beatMap instanceof Map?d.beatMap:null,checks={patchId:PATCH_ID==="alpha_kakashi_w2c_ending_35770_v10_2026_09_22",authorities:AUTHORITY.scene06==="820917031000c15e62f0a4cea7535397d3ad9e50"&&AUTHORITY.scene07==="2bcb6bb07dc862cb6f259b784782850619a08a4f"&&AUTHORITY.scene08==="275667ff8b65d5d7e9252164f7552676339a9086",aftermathExact:AFTERMATH_CUES.length===11&&AFTERMATH_CUES[0].text==="Petals drift across the stone."&&AFTERMATH_CUES[10].text==="Kakashi leaves the Sakura tree behind.",scene07Exact:SCENE07_CUES.length===23&&SCENE07_CUES[16].text==="I killed her."&&SCENE07_CUES[18].text==="Those weren’t your orders."&&SCENE07_CUES[19].text==="That was my choice.",scene08Exact:SCENE08_CUES.length===33&&SCENE08_CUES[20].text==="And killed one of us on the way."&&SCENE08_CUES[32].text==="I won’t have trouble with that.",beats:!!m&&[AFTERMATH,SCENE07,SCENE08,RECEIPT,EXIT].every(x=>m.has(x)),deathBeforeAnimation:killLedger.toString().includes("confirmedKill")&&beginConfirmedKill.toString().includes("killLedger"),sharedAttemptAnimation:playLethalAttemptAnimation35770.toString().includes("document.body.appendChild")&&playLethalAttemptAnimation35770.toString().includes('classList.add("is-active")')&&installStyle.toString().includes("2147483400")&&typeof globalThis.playAcademyKakashiLethalAttemptAnimation35770==="function",miPostKillCardDrop:removeDeadMiCardAfterKill.toString().includes("sc-kakashi-mi-death-drop-35770")&&installStyle.toString().includes("miCardDrop35770"),officeNarrativeStaging:installStyle.toString().includes('data-sc-kakashi-w2c-stage=\\\"office\\\"')&&installStyle.toString().includes('data-w2c-stage=\\\"office\\\"')&&installStyle.toString().includes("left:28%!important")&&installStyle.toString().includes("left:54%!important")&&installStyle.toString().includes("left:67%!important")&&installStyle.toString().includes("bottom:23.5%!important")&&!installStyle.toString().includes("sc-dialogue"+"-panel-33910")&&installStyle.toString().includes("width:max-content!important")&&board.toString().includes("sc-live-state-callout-33900")&&board.toString().includes("RECOVERED · HIDDEN OPERATION")&&board.toString().includes('card(MINATO,"MINATO","Assets/Kage/kage_minato.png","HOKAGE"')&&!board.toString().includes("sc-kakashi-w2c-package-status"),officeAnbuSlide:board.toString().includes("sc-w2c-office-anbu-enter-35770")&&installStyle.toString().includes("w2cOfficeAnbuEnter35770"),dialogueProjectionDelegated:!render.toString().includes(".sc-story-text")&&!render.toString().includes(".sc-story-name"),hiddenKnowledgeBoundary:commitHiddenReview.toString().includes("kakashiKnowledgeGranted:false"),canonicalReceiptHandoff:openReceipt.toString().includes("enterCanonicalReceipt")&&!openReceipt.toString().includes("document.body.appendChild"),konohaExitDelegated:completeToKonoha.toString().includes("retired_to_canonical_terminal_35100"),sharedSceneCut:sharedSceneCut35770.toString().includes("performStorySceneCut33900"),finalCuePostDelegate:globalThis.advanceStoryScene.toString().includes("confirmedKillPresentationStarted"),browserFinalCueCapture:installBrowserFinalCueCapture.toString().includes("stopImmediatePropagation")&&installBrowserFinalCueCapture.toString().includes("p.atEnd!==true"),directScene06Entrypoint:typeof globalThis.beginAcademyKakashiConfirmedKill35770==="function",confirmedKillWatchdog:installOutcomeWatchdog.toString().includes("beginConfirmedKill")&&installOutcomeWatchdog.toString().includes("120"),pursuitPreservedKillNotHijacked:beginConfirmedKill.toString().includes("kakashiScene06AW2CPursuitAvailable===true")&&installOutcomeWatchdog.toString().includes("kakashiScene06AW2CPursuitAvailable===true"),nonKillOutcomeDispatch:installOutcomeWatchdog.toString().includes("beginAcademyKakashiResolvedOutcome35780")&&showQaOutcome.toString().includes("presentation owner unavailable"),browserGoldenClaimed:false};const failed=Object.entries(checks).filter(([k,v])=>k!=="browserGoldenClaimed"&&v!==true).map(([k])=>k);return{pass:failed.length===0,checks,failed,browserGoldenClaimed:false};}
