...
const result=R(
  "kakashi_original_retrieval_resolution_33800","academy_kakashi",occurrence,
  ctx=>({
    ...
    packageInstanceRef:"kakashi_origin_outer_route_packet",originalObservationAction:ctx.kakashiOriginalAction||null,observerKnowledgeBasis:ctx.kakashiOriginalAction||null,retrievalIntelligenceClass:null,qualifyingParticipantOrCustodyInteractionOccurred:true,
    ...
    custodyFacts:ctx.kakashiRetrievalChoice==="secure_package"?["original_carrier_initial_holder","package_transfers_to_second_man","second_man_transfers_to_kakashi"]:["original_carrier_initial_holder","package_transfers_to_second_man","kakashi_pursues_original_target","package_not_recovered_by_kakashi"]
  }),
...
A.register({
  sceneId:scene,eventId:scene,title:"ACADEMY KAKASHI",entryBeatId:"kak_original_rooftop",participants:[],
  beats:[
    ...
    {beatId:"kak_original_transfer",mode:"narration",environmentRef:sakura,presentationResolver:()=>{
      ...
      const reaction={observe:"Kakashi stays still and watches the exchange unfold.",get_closer:"Kakashi closes the distance, using the alley's cover to keep the exchange in sight.",attack:"Kakashi moves early, forcing both men to react before the exchange can remain quiet.",attempt_pickpocket:"Kakashi slips toward the handoff, trying to take the package before either man can settle possession."}[action]||"Kakashi commits to the opening.";
...
try{const active=typeof getActiveStorySceneRuntime==="function"?getActiveStorySceneRuntime():null;if(active&&active.sceneId===scene&&typeof renderStoryScenePresentationLayer==="function")renderStoryScenePresentationLayer();}catch(_error){}
...
function runAlphaKakashiOriginal33800Diagnostics(){
  ...
  const checks={
    patchId:PATCH_ID==="alpha_kakashi_original_origin_33800_2026_09_13",authorityPinned:AUTHORITY_COMMIT==="04efa8c7b3faa5ec71b0191dcb673ebf3a60bbba",choiceLabelAuthorityPinned:CHOICE_LABEL_AUTHORITY_COMMIT==="21e0574c7efeb2257924d581370225ad92ab4fe9",rooftopEntry:!!d&&d.entryBeatId==="kak_original_rooftop",
    originalActionFamily:!!action&&action.choices.map(c=>c.choiceId).join("|")==="observe|get_closer|attack|attempt_pickpocket",modernizedActionLabels:!!action&&action.choices.map(c=>c.label).join("|")==="WATCH THE EXCHANGE|MOVE IN CLOSER|STRIKE BEFORE THE HANDOFF|SLIP IN FOR THE PACKAGE",originalMajorChoiceFamily:!!major&&major.choices.map(c=>c.choiceId).join("|")==="fight_assassin|secure_package|defeat_assassin_then_recover|pursue_original_target",modernizedMajorChoiceLabels:!!major&&major.choices.map(c=>c.label).join("|")==="CUT HER OFF|GO FOR THE PACKAGE|DEAL WITH HER FIRST, THEN CHASE THE PACKAGE|STAY ON THE FIRST MAN",noSubstituteEvaluator:!!m&&!Array.from(m.values()).some(b=>String(b.speakerName||"").includes("EVALUATOR")),
    kakashiSpecificBackdropSequence:!!rooftopBeat&&!!tailBeat&&!!transferBeat&&rooftopBeat.environmentRef&&rooftopBeat.environmentRef.assetId===rooftop.assetId&&tailBeat.environmentRef&&tailBeat.environmentRef.assetId===alley.assetId&&transferBeat.environmentRef&&transferBeat.environmentRef.assetId===sakura.assetId,
    exactBackdropPathsBound:!!getPath&&getPath(rooftop.assetId)===backdropPaths[rooftop.assetId]&&getPath(alley.assetId)===backdropPaths[alley.assetId]&&getPath(sakura.assetId)===backdropPaths[sakura.assetId],
    dedicatedBackdropActuallyResolvable:!!priorEnvironmentResolver33800&&rooftopProjection&&rooftopProjection.mode==="dedicated_backdrop"&&rooftopProjection.asset_id===rooftop.assetId&&rooftopProjection.asset_path===backdropPaths[rooftop.assetId]&&rooftopProjection.world_identity_unchanged===true,browserGoldenClaimed:false
  };
...
(function activateStorySceneBoard33900AfterKakashi33800(){
  ...
  const BUILD="scene-board-20260917-5";
  function load33920(){if(globalThis.SC_ALPHA_KAKASHI_STORY_PRESENTATION_COMPAT_33920||document.getElementById(COMPAT_ID))return;const compat=document.createElement("script");compat.id=COMPAT_ID;compat.src=`runtime/alpha-kakashi-story-presentation-compat-33920.js?v=${BUILD}`;compat.async=false;document.head.appendChild(compat);}
  function load33910(){if(globalThis.SC_KAKASHI_SCENE_BOARD_POLISH_33910){load33920();return;}const existing=document.getElementById(POLISH_ID);if(existing){existing.addEventListener("load",load33920,{once:true});return;}const polish=document.createElement("script");polish.id=POLISH_ID;polish.src=`runtime/alpha-kakashi-scene-board-polish-33910.js?v=${BUILD}`;polish.async=false;polish.addEventListener("load",load33920,{once:true});document.head.appendChild(polish);}
  if(globalThis.SC_STORY_SCENE_BOARD_33900){load33910();return;}
  const existing=document.getElementById(SCRIPT_ID);if(existing){existing.addEventListener("load",load33910,{once:true});return;}
  const script=document.createElement("script");script.id=SCRIPT_ID;script.src=`runtime/alpha-story-scene-board-33900.js?v=${BUILD}`;script.async=false;script.addEventListener("load",load33910,{once:true});document.head.appendChild(script);
})();
