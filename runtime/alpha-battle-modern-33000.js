  ...
  */
  ...
  if(typeof document!=="undefined"&&!document.getElementById("alpha-battle-modern-33000-style")){
    ...
    style.textContent=`
      ...
      .battle2-performance-host{position:absolute;left:44%;right:44%;top:15%;height:16%;z-index:32;pointer-events:none;overflow:visible}
      .battle2-performance-stage{position:absolute;inset:0;display:flex;align-items:flex-start;justify-content:center;isolation:isolate;transition:opacity .12s ease,transform .12s ease}
      .battle2-performance-center{width:100%;box-sizing:border-box;display:flex;min-height:66px;flex-direction:column;align-items:center;justify-content:center;padding:7px 8px;border-top:1px solid rgba(217,176,77,.34);border-bottom:1px solid rgba(217,176,77,.34);background:linear-gradient(90deg,transparent,rgba(2,8,12,.42) 15%,rgba(2,8,12,.48) 85%,transparent);text-align:center;text-shadow:0 2px 4px rgba(0,0,0,.9)}
      .battle2-performance-center small{color:#5fd9e2;font-size:6px;font-weight:900;letter-spacing:.08em;line-height:1.25}.battle2-performance-center strong{margin-top:5px;color:#f0e4ca;font:900 clamp(11px,.9vw,15px)/1.08 Georgia,serif}
      ...
      .battle2-modern.battle2-performance-active .battle-live-active-card{transition:filter .14s ease,opacity .14s ease,transform .14s ease!important}
