...
const PATCH_ID="alpha_kakashi_post_mi_death_pursuit_35830_v11_2026_09_19";
...
const AMT_ALLEY_ASSET_ID="kakashi_origin_pakkun_interception_alley",PS_ALLEY_ALT_ASSET_ID="kakashi_origin_konoha_alleyway_alt_night",TERMINAL_PENDING_BEAT="kak_seq_debrief_pending";
...
const BG=Object.freeze({sakura:"Kakashi Origin Backdrop/fight_at_sakura_tree.png",rooftop:"Kakashi Origin Backdrop/rooftop_night.png",psStreet:"Kakashi Origin Backdrop/konoha_alleyway_alt_night.png",amtStreet:"Kakashi Origin Backdrop/alleyway_konoha_night.png"});
...
function materializeAmtDecision(){
 ...
 const rows=[
  ...
  choice("postmi_amt_kill","KILL HIM",direct?BEAT.amtKill:TERMINAL_PENDING_BEAT,direct?()=>({success:true}):()=>deterministicKillTarget(AMT,"amt")),
...
function installBeats(){
 const d=scene(),m=d&&d.beatMap instanceof Map?d.beatMap:null;if(!m)return false;
 try{if(typeof registerSceneBackdropAssetPath==="function")registerSceneBackdropAssetPath(PS_ALLEY_ALT_ASSET_ID,BG.psStreet);}catch(_e){}
 ...
 narr(BEAT.psChase,PS_ALLEY_ALT_ASSET_ID,OBJECTIVE.ps);narr(BEAT.psFail,PS_ALLEY_ALT_ASSET_ID,OBJECTIVE.report);narr(BEAT.psCatch,PS_ALLEY_ALT_ASSET_ID,OBJECTIVE.ps);
 m.set(BEAT.psBattle,{beatId:BEAT.psBattle,mode:"battle_transition",environmentRef:{assetId:PS_ALLEY_ALT_ASSET_ID},text:"",battle:{encounterId:PS_CONFIG,launchResolver:ctx=>launchPs(ctx),postBattleBeatId:BEAT.psReturn,resultProjector:projector,actionLabel:"STOP THE PACKAGE SMUGGLER"},exitScene:false,allowPresentationClose:false,choices:[]});
 narr(BEAT.psReturn,PS_ALLEY_ALT_ASSET_ID,OBJECTIVE.ps);m.get(BEAT.psReturn).onEnterConsequences=[{requestId:"postmi_35830_ps_return_consume",kind:"domain",resolve:()=>consumeReturnOnEnter35830("ps")}];narr(BEAT.psWin,PS_ALLEY_ALT_ASSET_ID,OBJECTIVE.ps);narr(BEAT.psLoss,PS_ALLEY_ALT_ASSET_ID,OBJECTIVE.report);
 m.set(BEAT.psDecision,{beatId:BEAT.psDecision,mode:"choice",environmentRef:{assetId:PS_ALLEY_ALT_ASSET_ID},objectiveText:OBJECTIVE.ps,text:"Package Smuggler is down. The package is secure. Kakashi decides what remains worth pursuing.",nextBeatId:null,exitScene:false,allowPresentationClose:false,choices:[]});
 narr(BEAT.psReport,PS_ALLEY_ALT_ASSET_ID,OBJECTIVE.report);
...
function transitionNarrative(rt){
 ...
 else if(rt.beatId===BEAT.amtKill){const k=deterministicKillTarget(AMT,"amt");if(!k.success)return k;rt.beatId=TERMINAL_PENDING_BEAT;}
...
function positionCurrentDialogue35830(layer){
 const panel=layer&&layer.querySelector&&layer.querySelector(".sc-performance-surface-33910 .sc-dialogue-panel-33910.is-current");if(!panel)return false;
 const pakkun=String(panel.dataset&&panel.dataset.speakerId||"")==="pakkun";
 const set=(name,value)=>{try{panel.style.setProperty(name,value,"important");}catch(_e){}};
 if(pakkun){set("left","auto");set("right","6%");set("top","17%");set("bottom","auto");set("transform","none");set("width","min(27%,380px)");}
 else{set("left","50%");set("right","auto");set("top","-6%");set("bottom","auto");set("transform","translateX(-50%)");set("width","min(31%,430px)");}
 return true;
}
...
function render(){
 ...
 positionCurrentDialogue35830(layer);
 return true;
}
...
function diagnostics(){
 ...
 const checks={
  ...
  exactBattleConfigs:PS_CONFIG==="academy_kakashi_origin_battle_seq_ps"&&AMT_CONFIG==="academy_kakashi_origin_battle_seq_amt_pakkun",
   ...
   psBackdropExact:BG.psStreet==="Kakashi Origin Backdrop/konoha_alleyway_alt_night.png"&&PS_ALLEY_ALT_ASSET_ID==="kakashi_origin_konoha_alleyway_alt_night"&&installBeats.toString().includes("registerSceneBackdropAssetPath(PS_ALLEY_ALT_ASSET_ID,BG.psStreet)"),
   ...
   postMiDialogueClearsActorCards:installStyle.toString().includes("top:-6%!important")&&positionCurrentDialogue35830.toString().includes('style.setProperty(name,value,"important")')&&render.toString().includes("positionCurrentDialogue35830(layer)"),
  ...
  packageSecuredAmtKillHasLiveSuccessor:materializeAmtDecision.toString().includes("direct?BEAT.amtKill:TERMINAL_PENDING_BEAT")&&transitionNarrative.toString().includes("rt.beatId=TERMINAL_PENDING_BEAT"),
