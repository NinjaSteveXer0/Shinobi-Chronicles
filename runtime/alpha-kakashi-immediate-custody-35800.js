...
function eligibleSource(rt){rt=rt||active();const local=String(rt&&rt.localContext&&rt.localContext.kakashiScene05AWMiStateClass||""),miState=local||currentMiState();return !!rt&&rt.sceneId===SCENE_ID&&rt.beatId===SOURCE_BEAT&&rt.localContext&&rt.localContext.kakashiScene05AWEntered===true&&miState!=="DEAD";}
...
function board(rt,p){
  ...
  const actors=[card(MINATO,"MINATO","Assets/Kage/kage_minato.png","HOKAGE",f===MINATO),card(AMT,"ANBU MARKED TARGET","NPC/anbu_marked_target.png","ALIVE",f===AMT),miPresent?card(MI,"MASKED INTERCEPTOR","NPC/masked_interceptor.png","ANBU CUSTODY",f===MI):"",card(ANBU,"ANBU OPERATIVE","NPC/konoha_anbu.png","PRESENT",f===ANBU),card(PS,"PACKAGE SMUGGLER","NPC/package_smuggler.png","ALIVE",f===PS)].join("");
  return'<div class="sc-scene-board-33900__top"><div class="sc-scene-board-33900__location">HOKAGE ADMINISTRATION · NIGHT</div></div><div class="sc-scene-board-33900__actors" data-count="'+(miPresent?5:4)+'">'+actors+'</div><div class="sc-scene-board-33900__objects sc-live-state-callouts-33900"><span class="sc-scene-board-33900__object sc-live-state-callout-33900 is-committed"><b>PACKAGE</b>RECOVERED · HIDDEN OPERATION</span></div>';
}
function installStyle(){
  ...
  "."+BOARD_CLASS+" .sc-scene-board-33900__actor{width:min(19vw,270px)!important;max-height:440px!important;aspect-ratio:7/10!important}"+
  ...
  "#story-scene-presentation-layer[data-sc-kakashi-custody-stage='rooftop_custody'] .sc-performance-surface-33910 .sc-dialogue-panel-33910.is-current,#story-scene-presentation-layer[data-sc-kakashi-custody-stage='rooftop_report'] .sc-performance-surface-33910 .sc-dialogue-panel-33910.is-current,#story-scene-presentation-layer[data-sc-kakashi-custody-stage='police'] .sc-performance-surface-33910 .sc-dialogue-panel-33910.is-current{left:50%!important;right:auto!important;top:22%!important;bottom:auto!important;transform:translateX(-50%)!important;width:min(40%,560px)!important;max-width:560px!important}"+
